...
OUTDIR = Path(
    r"C:\Users\brend\Desktop\Project Data\Combined final dataset\Supplementary\Final Benchmarking Results\Panel Comparison\Internal\LTB v CON"
)
...
MODEL_NAME = "MLP"
MODEL = MLPClassifier(hidden_layer_sizes=(50,), max_iter=500, random_state=SEED)
...
def compute_metrics(y, p):
    ...
    return auc, brier, sens, spec, bal_acc
...
if INCLUDE_RANDOM_PANEL:
    rng = np.random.default_rng(SEED)
    random_genes = set(rng.choice(list(all_genes), RANDOM_PANEL_SIZE, replace=False))
    PANELS[f"Random{RANDOM_PANEL_SIZE}_seed{SEED}"] = random_genes

panel_curves = {}

for panel_name, panel_source in PANELS.items():

    print(f"\n=== PANEL: {panel_name} | TASK: {TASK_NAME} ===")

    if isinstance(panel_source, str):
        panel = pd.read_csv(panel_source)
        panel["Gene"] = panel["Gene"].apply(normalize_gene)
        panel_genes = set(panel["Gene"])
    else:
        panel_genes = panel_source
    ...
    for fold, (train_files, test_files) in enumerate(loco_splits(TRAIN_FILES), start=1):
        ...
        p_test = MODEL.predict_proba(X_test.to_numpy())[:, 1]
    ...
    panel_curves[panel_name] = (y_all, p_all)

    auc, brier, sens, spec, bal_acc = compute_metrics(y_all, p_all)

    print(f"AUC={auc:.3f} | BalAcc={bal_acc:.3f}")

    run = f"{TASK_NAME}_{panel_name}_INTERNAL_LOCO"
...
for panel_name, (y, p) in panel_curves.items():
...
plt.title(f"{TASK_NAME} — Internal LOCO (Panels, MLP)")
...
plt.savefig(
    OUTDIR / f"{TASK_NAME}_INTERNAL_PANELS_ROC.tiff",
    dpi=600,
    bbox_inches="tight"
)
...
print("\n PANEL INTERNAL LOCO COMPLETE — publication-grade ROC exported")